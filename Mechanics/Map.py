...
class AdjacentMap:
    ...
    def adjacent_walkable(self, x, y, dim):
        ...
        tiles = [t for t in potential_tiles if
                 self.game.state.data['tile_collision'][t[0]][t[1]] == MapC.WALKABLE and
                 self.game.state.data['unit'][t[0]][t[1]] == Unit.NONE
                 ]
...
class Map:
    ...
    def __init__(self, game):
        ...
        # Map Data, Do not alter
        self.orig_tiles = None
        self.orig_tile_collision = None
        self.orig_spawn_locations = None
    ...
    def preload(self, map_name):
        ...
        self.coord_map = np.array([[(x, y) for x in range(self.width)] for y in range(self.height)])

        logging.debug("Loaded %s, a %sX%s sized map!" % (map_name, self.height, self.width))

    def load(self):
        self.orig_spawn_locations = []
        self.orig_tile_collision = np.zeros((self.height, self.width), dtype=np.int)
        self.orig_tiles = np.zeros((self.height, self.width), dtype=np.int)
        ...
        for y, val in enumerate(self.raw_data):
            for x, tile_id in enumerate(val):
                ...
                if tile_id == MapC.SPAWN_POINT:
                    self.orig_spawn_locations.append((x, y))
                    tile_id = MapC.GRASS

                tile = MapC.TILE_DATA[tile_id]

                self.orig_tiles[x][y] = tile_id
                self.orig_tile_collision[x][y] = tile['type']

    def setup_spawn_locations(self):
        self.spawn_tiles = self.orig_spawn_locations.copy()

    def tile_collision(self):
        return self.orig_tile_collision

    def tiles(self):
        return self.orig_tiles
