...
class MapLoader:
    ...
    def preload(map_name):
        ...
        for y, val in enumerate(raw_data):
            for x, tile_id in enumerate(val):
                ...
                if tile_id == const.Map.ID_SPAWN_POINT:
                    MapLoader.spawn_tiles.append((x, y))
                    tile_id = const.Map.ID_GRASS
    ...
    def _base_vision():
        ...
        if Config.NO_FOG:
            ...
            tiles = np.nonzero(MapLoader.tiles == const.Map.ID_WALL, dtype=np.int8)
    ...
    def is_harvestable_tile(unit, x, y):
        ...
        tile = const.Map.TILE_DATA[tile_id]

        return tile['type'] == const.Map.TYPE_HARVESTABLE

    @staticmethod
    def is_walkable_tile(unit, x, y):
        tile_walkable = MapLoader.tiles[x][y] == const.Map.TYPE_WALKABLE
        unit_walkable = unit.game.data['unit'][x, y] == const.Unit.ID_NONE
    ...
    def is_attackable(unit, x, y):
        ...
        return unit_data != const.Unit.ID_NONE and unit_player != unit.player.id
