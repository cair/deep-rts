...
def verify(files):
    ...
    for file in tqdm(files):
        log_item = load_buffer(file)
        
        try:
            log_item.GameNum()
            log_item.Items(0).Items(0)
            newlist.append(file)
        except:
            os.remove(file)
...
if __name__ == "__main__":
    # Plot Data members
    plots = []
    average_plots_d = {}  # Collection of average plotting per player
        ...
        for game in data:
            ...
            if plot_average:
                if playerID not in average_plots:
                    average_plots_d[playerID] = [[0] * len(data_x), [0] * len(data_y)]

                for point in range(len(data_x)):
                    average_plots_d[playerID][0][point] += data_x[point]
                    average_plots_d[playerID][1][point] += data_y[point]

        num_processed += 1

    if plot_average:
        for playerID, value in average_plots_d.items():
